...
`synchronized`关键字，它配合Object 的`wait`、`notify`系列方法可以实现等待/通知机制。对于Lock，通过Condition也可以实现等待/通知模式
...
Lock只能实现互斥（一个线程持有锁，另外的线程不能访问该锁），但是不能实现通信。而Condition可以实现线程之间的合作通信，即使获取了CPU的执行权，但是也可以让出执行权通知另外的线程执行。
